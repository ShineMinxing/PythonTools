...
def main():
    ...
    input_dir = os.path.join(script_dir, 'local_file')
    ...
    for filename in os.listdir(input_dir):
        # 只处理.txt文件且不以RP结尾的文件
        if filename.endswith('.txt') and not filename.lower().endswith('rp.txt'):
            input_path = os.path.join(input_dir, filename)
            
            # 创建输出文件名 (在原文件名后添加RP)
            base_name = os.path.splitext(filename)[0]
            output_filename = f"{base_name}RP.txt"
